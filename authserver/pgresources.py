        ...
        if user_validation['isValid']:
            try:
                ...
                run_db_query('call spUserInsertUpdate ('
                             ...
                             args, 'user register in DB')

                return {'message': 'user registration'}, 200
    ...
    def post(self):
        ...
        if user_login_validation['isValid']:
            try:
                ...
                result = run_db_query('SELECT user_id, user_password from public."users" WHERE '
                                      'user_emailaddr=%(email)s', args, 'user login in DB')

                if not result:
                    return {"message": "Invalid credentials"}, 500
                password = data['password'].encode('utf-8')
                if bcrypt.check_password_hash(result['user_password'], password):
                    access_token = create_access_token(identity={'email': args['email'], 'id': result['user_id']},
